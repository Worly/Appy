...
namespace Appy.Tests.Services
{
    public class TimeOffServiceTests
    {
        ...
        [Fact]
        public void OrderRecurringByNextOccurrence_SortsByNextOccurrence_NoOccurrenceLast()
        {
            ...
            // Has a future end bound but no remaining occurrence: Sunday rule ending the Wednesday before its next Sunday.
            var dead = new TimeOff { Id = 3, Recurrence = TimeOffRecurrence.Weekly, DayOfWeek = DayOfWeek.Sunday, EndDate = new DateOnly(2030, 6, 12) };

            var ordered = TimeOffService.OrderRecurringByNextOccurrence(new[] { weds, dead, tue }, Today);

            Assert.Equal(new[] { 2, 1, 3 }, ordered.Select(t => t.Id).ToArray()); // Tue, Wed, then the no-occurrence rule last
        }

        // ---- GetList (DB-side filter + ordering + paging) ----

        [Fact]
        public async Task GetList_OneOffActive_ExcludesExpiredAndSortsByStartDate()
        {
            var today = DateOnly.FromDateTime(DateTime.Today);
            Seed(new TimeOff { Id = 1, Recurrence = TimeOffRecurrence.OneOff, StartDate = today.AddDays(-5), EndDate = today.AddDays(10) });   // ongoing
            Seed(new TimeOff { Id = 2, Recurrence = TimeOffRecurrence.OneOff, StartDate = today.AddDays(20), EndDate = today.AddDays(25) });  // future
            Seed(new TimeOff { Id = 3, Recurrence = TimeOffRecurrence.OneOff, StartDate = today.AddDays(-40), EndDate = today.AddDays(-31) }); // past
        ...
        [Fact]
        public async Task GetList_RecurringActive_IncludesOpenEnded_ExcludesOneOffs()
        {
            var today = DateOnly.FromDateTime(DateTime.Today);
            Seed(new TimeOff { Id = 1, Recurrence = TimeOffRecurrence.Monthly, DayOfMonth = 15, StartDate = today.AddDays(-100) }); // open-ended recurring
            Seed(new TimeOff { Id = 2, Recurrence = TimeOffRecurrence.OneOff, StartDate = today, EndDate = today.AddDays(30) });
        ...
        public async Task GetList_AppliesSkipAndTake()
        {
            ...
            foreach (var i in Enumerable.Range(1, 5))
                Seed(new TimeOff { Id = i, Recurrence = TimeOffRecurrence.OneOff, StartDate = today.AddDays(i), EndDate = today.AddDays(30 + i) });

            var page = await service.GetList(TimeOffListType.OneOff, TimeOffScope.Active, 2, 2, FacilityId);
