...
**List view** (`appointments-list/`): A chronological list with date dividers. Paginated via a `PagedResult` (from `pagedQuery`) — loads 20 items per page, supports infinite scroll both forwards and backwards from the current date anchor. Maintains scroll position when loading more items. Each date divider and the sticky current-date header also show a relative-date label (Today / Tomorrow / in N days / N days ago), coloured by past/today/future via the shared `relativeDate`/`dateRelation` pipes. Between two adjacent appointments **on the same day**, the list shows the idle time as a faint dashed divider with the duration centred on it; when they overlap, the divider becomes a red jagged rule with a warning icon and both cards get a red inset outline. This comes from a `"gap"` rendered-item type built in `renderAppointments()` via the pure `timeBetweenMs()` helper (`utils/time-utils.ts`); only consecutive same-day pairs are compared (a long appointment overlapping a non-adjacent later one is not flagged).
