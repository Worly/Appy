...
- **`SingleAppointmentListItemComponent`**: One row in the list view, showing the from–to time, the appointment duration (faint, just after the time — issue #128), service, and client. Takes an `isOverlapping` input that draws a red inset outline when the appointment collides with an adjacent one, and an `isOnDayOff` input that draws the same outline when the appointment falls on a full-day off.
