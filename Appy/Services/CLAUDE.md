...
| `HolidayService` | Holiday import: `GetSettings` / `SaveSettings` (future-only reset + immediate materialization, provider-down aborts before any DB mutation) / `Materialize` (idempotent window fill) / `GetSupportedCountries`. Import window is today → today+1yr; past rows are never deleted. List/edit/remove/revert in Tasks 4–5. |
