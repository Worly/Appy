...
- Preserving a facility's edits/removals across a country change (future ones are discarded — see *Configure flow*).
...
**Changing or clearing the country** removes only the facility's **future** imported holidays (and any edits/removals on them); **past holidays stay as history** (so the history can hold holidays from a previously-configured country — see *Data model*). Show a confirmation first.
...
Reuses the shared **Upcoming | History** scope switch. Upcoming = holidays on/after today; History = past. Rows sort by **effective date** (the current date for active/edited rows, the original date for removed rows). After the country is turned off, Upcoming shows the **"Configure"** empty state while History still lists the retained past holidays.

Tapping any row opens the details view (or, for a removed holiday, the restore confirmation — see *Remove / restore*).
...
- A provenance line: `🏖 Public holiday · Croatia` (the country comes from the holiday's own `CountryCode`).
...
- **Removed holidays don't open this view** — tapping one goes straight to a restore confirmation (see *Remove / restore*).
...
HolidayImportSettings (per facility)
  ...
  CountryCode           the currently-configured ISO country code; null ⇒ not configured

ImportedHoliday (per facility, per holiday occurrence)
  Id
  FacilityId            (FK)
  CountryCode           ISO country this holiday was imported from — history may hold
                        holidays from a previously-configured country, so country is
                        stored per-occurrence rather than read from settings
  Name                  holiday name / TimeOff label (the source's localName)
  Date                  the source-computed date — IMMUTABLE; the import job's match key
                        together with FacilityId + CountryCode
  (the snapshot is just Date + Name: imported holidays are always all-day with no
   note, so no other original fields are needed)
...
Each holiday returns `date`, `localName`, `name`, `countryCode`, `global`, `subdivisionCodes`, and `types`, so we get the localized name directly. We store its `localName` as the holiday `Name` (always the country's own language, independent of the app's UI language) and its `countryCode` on the `ImportedHoliday`.
...
- **Revert** (edited holiday) — on the existing linked TimeOff, reset `StartDate`/`EndDate` to the `ImportedHoliday.Date` and set `IsAllDay = true` (clear the time range); **leave `Notes` unchanged**.
- **Restore** (removed holiday) — recreate the linked TimeOff from the snapshot (all-day on `ImportedHoliday.Date`, label `Name`, no notes); removal already discarded any note.
...
- **Models:** a `Holiday` view model (name, country, effective date, all-day/time, notes, `isEdited`, `isRemoved`, and the original date for the Changes block) + a settings model.
- **Service:** a holiday service for list / edit / remove / revert / restore / get-settings / save-settings / supported-countries, invalidating the relevant cache keys.
- **Components — extended, not added:**
  ...
  - `app-single-time-off` — provenance line, Changes block + Revert, Remove/Edit; reused as-is by the appointments view.
...
| 4 | Remove | Remove (confirm) deletes the linked TimeOff; "Removed" treatment in list |
